...
def plotSimple():
    ...
    for i in range(-5, 6, 2):
        for j in range(-5, 6, 2):
            ...
            input[0][0] = i
            input[0][1] = j
            learner.plot_prediction(str(i)+"_"+str(j), input)
...
def plotAmplified():
    ...
    for i in range(-5, 6, 2):
        for j in range(-5, 6, 2):
            ...
            result[2:] = learner.model.predict(input)

            full_result.append(result)

    full_result_np = np.asarray(full_result)
    full_result_np = full_result_np[:,2:]
    ...
    f = open(ll_activation+"_"+loss_fun+"/predictions_scaled/_info_"+str(round(full_result_min,3))+\
    ...
    for item in full_result:
        ...
        title = str(item[0])+"_"+str(item[1])

        f.write("\nsum " + title + " : " + str(np.sum(item[2:])))

        result_modified = np.reshape(np.asarray(item[2:]*255/median), (9,9))
        n = 9
